...
| `--swarm-key <hex\\|@file>` | off | private swarm mode: trusted publisher key, implies `--dht`, ignores `--bootstrap`, never falls back to the Hub (manifest must be embedded in the record or available out-of-band) |
